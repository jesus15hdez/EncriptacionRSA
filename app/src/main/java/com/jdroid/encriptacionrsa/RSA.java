...
public class RSA {
	...
	public void saveToDiskPrivateKey(File path, String name[]) throws IOException {
		try {
			File ruta_sd = path;
			File f = new File(ruta_sd.getAbsolutePath(), name[0]+"Key.pri");
			OutputStreamWriter fout = new OutputStreamWriter(new FileOutputStream(f));
			fout.write(this.getPrivateKeyString()+"."+name[1]);
			fout.close();
		} catch (Exception e) {
	...
	public void saveToDiskPublicKey(File path, String name) {
		try {
			File ruta_sd = path;
			File f = new File(ruta_sd.getAbsolutePath(), name+"Key.pub");
			OutputStreamWriter fout = new OutputStreamWriter(new FileOutputStream(f));
			fout.write(this.getPublicKeyString());
			fout.close();
		} catch (Exception e) {
			// TODO: handle exception
		}
	...
	public void openFromDiskPublicKey(File path, String name) throws IOException, NoSuchAlgorithmException, InvalidKeySpecException {
		String content = this.readFileAsString(path, name);
		this.setPublicKeyString(content);
	}
	
	public String openFromDiskPrivateKey(File path, String name) throws IOException, NoSuchAlgorithmException, InvalidKeySpecException {
		String content[] = this.readFileAsString(path, name).split("\\.");
		this.setPrivateKeyString(content[0]);
      return content[1];
	}
	
	
	private String readFileAsString(File path, String name) throws IOException {
		File f = new File(path.getAbsolutePath(), name);
